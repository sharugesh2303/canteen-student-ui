...
const OrderCard = ({ order, orderIndex }) => {
    ...
    return (
        // The link correctly uses the order's unique ID for navigation
        <Link 
            to={`/order-details/${order._id}`} 
            state={{ order: order }} // Pass full object as fallback/optimistic display data
            className="block" 
        >
    ...
    const fetchOrderHistory = async () => {
        ...
        try {
            ...
            if (err.response && err.response.status === 401) {
                ...
                setError('Failed to fetch order history. Check backend connection.');
            }
        } finally {
            setLoading(false);
        }
